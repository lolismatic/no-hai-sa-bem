	...
	public class InteractionTriggerInspector : Editor {
		...
		private void DrawCharacterPosition(InteractionTrigger.Range range, int index) {
			...
			if (direction != Vector3.zero && range.characterPosition.maxAngle < 180f) {
				...
				Handles.DotHandleCap(0, position + x, Quaternion.identity, range.characterPosition.radius * 0.01f, EventType.Repaint);
			}
		...
		private void DrawCameraPosition(InteractionTrigger.Range range, int index) {
			...
			Handles.DotHandleCap(0, position + direction, Quaternion.identity, 0.005f, EventType.Repaint);
			...
			Handles.CircleHandleCap(0, position + direction.normalized * d, rotation, r, EventType.Repaint);
