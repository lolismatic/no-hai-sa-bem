#pragma warning disable 0618 // the obsolete shit
using UnityEngine;
using UnityEditor;
	...
	public class IKSolverAimInspector: IKSolverInspector {
		...
		public static void AddScene(IKSolverAim solver, Color color, bool modifiable) {
			...
			for (int i = 0; i < solver.bones.Length; i++) {
				...
				Handles.SphereHandleCap(0, solver.bones[i].transform.position, Quaternion.identity, GetHandleSize(solver.bones[i].transform.position), EventType.Repaint);
			}
			...
			if (Application.isPlaying && solver.poleWeight > 0f) {
				if (modifiable) {
					Handles.SphereHandleCap(0, solver.polePosition, Quaternion.identity, GetHandleSize(solver.IKPosition) * 0.5f, EventType.Repaint);
...
#pragma warning restore  0618 // the obsolete shit
